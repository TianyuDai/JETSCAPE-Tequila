...
int main(int argc, char** argv)
{
  ...
  double jetpTMin = 50., jetRadius = 0.2, jetAbsRapMax = 2., partonpTMin = 2.;
  ...
  while (!reader->Finished())
  {
    ...
    if(TotalPartons > 0)
    {
      ...
      auto partons_for_jet = mShowers[0]->GetFinalPartonsForFastJet();

      for (int i=1; i < mShowers.size(); i++)
      {
        auto i_partons_for_jet = mShowers[i]->GetFinalPartonsForFastJet(); 
        partons_for_jet.insert(std::end(partons_for_jet), std::begin(i_partons_for_jet), std::end(i_partons_for_jet)); 
      }
      /*
      for (unsigned int i = 0; i < partons_for_jet.size(); i++)
      {
        auto iterator = partons_for_jet.begin() + i; 
        partons_for_jet.erase(iterator);  
      }
*/
      fjcore::ClusterSequence clustSeq(partons_for_jet, jetDef);

...
for i, new_pT_hat_min in enumerate(pTHat_list[:-1]): 
    for task in range(10): 
        with open('../config/STAT_AuAu200.xml', 'rb') as xml_file: 
            tree = ET.parse(xml_file)
            root = tree.getroot()
        
            name = root.find('outputFilename') 
            file_name = '/global/cscratch1/sd/td115/output/STAT/AuAu200/20220221/%.6f_i%d' %(new_pT_hat_min, task)
            name.text = file_name
            ...
            tree.write('../config_files/LBT_%.6f_i%d.xml' %(new_pT_hat_min, task), xml_declaration=True, encoding='utf-8')
